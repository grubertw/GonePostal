...
@implementation GPLocationSearch
...
- (void)loadSearchControllersFromPredicate {
    ...
    self.itemsInSearch = [NSMutableArray arrayWithCapacity:fetchedObjects.count];
    self.itemsNotInSearch = [NSMutableArray arrayWithCapacity:fetchedObjects.count];
    
    for (NSUInteger i=0; i<fetchedObjects.count; i++) {
        Location * item = [fetchedObjects objectAtIndex:i];
        bool matchFound = NO;
...
{
    self = [super initWithNibName:@"GPLocationSearch" bundle:nil];
    if (self) {
...
- (IBAction)saveSearch:(id)sender {
    ...
    for (NSUInteger i=0; i<selectedItems.count; i++) {
        Location * item = [selectedItems objectAtIndex:i];
