...
@implementation GPValuationCalculator
...
+ (NSString *)deriveCatalogValueOfStamp:(Stamp *)stamp {
    NSMutableString * valuationDesc = [[NSMutableString alloc] init];
    ...
    //
    NSFetchRequest * level1Fetch = [NSFetchRequest fetchRequestWithEntityName:@"Valuation"];
    [level1Fetch setPredicate:[NSPredicate predicateWithFormat:@"gpCatalog.gp_catalog_number like %@ and priceList.name like %@ and stampFormat.name like %@", stamp.gpCatalog.gp_catalog_number, priceList.name, stamp.format.name]];

    NSArray * level1Vals = [stamp.managedObjectContext executeFetchRequest:level1Fetch error:nil];
    
    // There should only be 1 item found in the fetch result.
    if (level1Vals && [level1Vals count] == 1) {
        Valuation * l1Val = level1Vals[0];
        stamp.catalog_value = l1Val.price;
    }
